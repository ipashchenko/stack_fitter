...
class Model(object):
    ...
    def from_prior(self):
        lg_sigma_M = np.random.normal(1, 1)
        ...
        return np.array([lg_sigma_M, M_BH_sun, D_Mpc, a])
    ...
    def perturb(self, params):
        ...
        # lg_sigma_M
        if which == 0:
            logH -= -0.5*((params[which] - 1.)/1.)**2
            params[which] += 1.*self.randh()
            logH += -0.5*((params[which] - 1.)/1.)**2
        # M_BH_sun
        elif which == 1:
    ...
    def log_likelihood(self, params):
        ...
        # In mas
        r1_model_mas = self.rad_to_mas*r1_model_pc/(1e+06*D)
        return -0.5*np.log(2*np.pi*r1_obs_sigma_mas**2) - 0.5*(r1_obs_mas - r1_model_mas)**2/r1_obs_sigma_mas**2
...
post_samples_use = np.vstack((post_samples[:, 0], post_samples[:, 1], post_samples[:, 2], np.log10(post_samples[:, 3]))).T
...
fig = corner.corner(post_samples_use,
                    ...
                    fig=fig)
fig.savefig("inference_using_r1.png", bbox_inches="tight", dpi=300)
plt.show()
